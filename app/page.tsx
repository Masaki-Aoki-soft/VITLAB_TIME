...
export default function Home() {
    ...
    const loadSavedRoutesList = async () => {
        try {
            const response = await fetch('/api/list-saved-routes');
    ...
    const drawAllRoutes = async (routes: RouteResult[]) => {
        ...
        const geojsonFolder = '/api/static/oomiya_line/';
        ...
        for (const route of uniqueRoutes) {
            ...
            for (const filename of segments) {
                try {
                    const response = await fetch(geojsonFolder + filename.trim());
        ...
        if (dataType === 'signal') {
            ...
            routeData.forEach((row) => {
                if (row.signal == 1) {
                    ...
                    const promise = fetch(geojsonFolder + filename)
                        .then((response) => {
    ...
    const viewAREA = async () => {
        try {
            const response = await fetch('/api/static/RE_AREA.geojson');
            if (!response.ok)
    ...
    useEffect(() => {
        ...
        for (let i = 1; i <= 300; i++) {
            ...
            const filePath = '/api/static/oomiya_point/' + fileName;
            pointPromises.push(
                fetch(filePath)
                    .then((response) => {
                        if (response.ok) {
